...
That second assignment is the interesting one — that's the moment
the planner decides "this is not a creative task, this is a
rule-following task, send it to the deterministic side."
...
loop, and the loop suggests… the same venue again. Or one that
also fails. Burning Nebius tokens on every round, no progress, the
